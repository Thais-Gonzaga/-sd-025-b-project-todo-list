...
const checkTodoList = (todos = []) => {
  if (todos.length === 0) {
    ...
    cy.get(TODO_LIST_LINE_SELECTOR).each(($li, index) => {
      ...
      else expect($li).to.not.have.class('completed');
    });
  }
};


describe('1 - Adicione à sua lista o título `Minha Lista de Tarefas` em uma tag `header`', () => {
  beforeEach(() => {
...
describe('6 - Adicione três novas tarefas e ordene todas as tarefas da lista por ordem de criação', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('./index.html');
  });
...
describe('7 - Permita que um item da lista ao ser clicado tenha sua cor de fundo alterada para cinza', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('./index.html');
  });
...
describe('8 - Não permita que seja possível selecionar mais de um elemento da lista ao mesmo tempo', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('./index.html');
  });
...
describe('9 - Ao clicar duas vezes em um item, faça com que ele seja riscado, indicando que foi completado. Deve ser possível desfazer essa ação clicando novamente duas vezes no item', () => {
  beforeEach(() => {
    cy.viewport(1366, 768);
    cy.visit('./index.html');
  });
